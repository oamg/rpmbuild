...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield gha_exec(`curl --location --progress-bar --output tmp.tar.gz https://api.github.com/repos/${owner}/${repo}/tarball/${ref}`);
            // create directory to match source file - %{name}-{version}.tar.gz of spec file
            yield gha_exec(`mkdir ${name}-${version}`);
            // Extract source code 
            yield gha_exec(`tar xf tmp.tar.gz -C ${name}-${version} --strip-components 1`);
            // Create Source tar.gz file 
            yield gha_exec(`tar -czf ${name}-${version}.tar.gz ${name}-${version}`);
